...
class QHeader:
    ...
    configHeaderIdent = 0x0827 # version 1.2 ident for configuration records
    ...
    def chkIdent(cls, fp) -> bool:
        ...
        return ident == cls.headerIdent # A header record identifier
    ...
    def __init__(self, fp, fn:str) -> None:
        ...
        if ident != 0x1729:
            raise ValueError(f"Invalid header identifer, {ident:#05x} != 0x1729, in {fn}")
        ...
        if self.version == QVersion.v12:
            # cfgIdent is bad in the beta version of 1.2, should be configHeaderIdent
...
def main():
    ...
    parser.add_argument("--config", action="store_false", help="Don't dsplay config")
